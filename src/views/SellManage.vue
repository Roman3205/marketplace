...
<template>
  <div class="container">
    <div class="wrapper">
      ...
      ></spinner-loading>
      <h5 class="mt-4" v-if="orders.length == 0 && !loading">
        Ваши товары еще никто не заказывал
      </h5>
      <div class="container-orders" v-if="orders.length != 0">
        <div class="sell-order" v-for="item in orders" :key="item">
          <div class="left-side-sell">
            <h2>
              Дата заказа: <b>{{ getTime(item.createdAt) }}</b>
            </h2>
            <p>
              Покупатель: <b>{{ item.customer_id.name }}</b>
            </p>
            <p>
              Название товара:
              <b
                >{{ item.product_id.brand_id.brandName }} /
                {{ item.product_id.title }}</b
              >
            </p>
            <p>
          ...
          <div class="right-side-sell">
            <h2>Статус:</h2>
            <div v-if="item.status === 'Создан'" class="alert alert-danger">
              Ожидает подтверждения
            </div>
            <div
              v-if="item.status !== 'Создан'"
              class="alert"
              :class="{
