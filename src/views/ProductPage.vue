...
export default {
  ...
  methods: {
    ...
    async CreateReview(evt) {
      ...
      if (
        !/^[А-Яа-я\s,'-.!" "?]+$/.test(this.inputValue) ||
        filter.test(this.inputValue)
      ) {
  ...
  <div class="container">
    ...
    >
      ...
      <div class="prod-title" v-if="product">
        <h2>
          <b>{{ product.brand_id.brandName }} / {{ product.title }}</b>
        </h2>
        <div class="info">
          <p v-if="product.averageRating > 0" class="star">
            <i class="fa fa-star"></i><span>{{ product.averageRating }}</span>
          </p>
          <p>
            <u>{{ product.amountReviews }} оценок</u>
          </p>
        ...
        <div class="prod-menu">
          <div class="brand">
            ...
            <div class="brand-info">
              <div class="sale">
                <i class="fa fa-thumbs-up"></i>{{ product.brand_id.sold
                }}<br />товаров продано
              </div>
              <div class="reg">
          ...
          <div class="prod-description" v-if="product">
            ...
            <div class="lock">
              ...
              ><span><u>Качество подтверждено</u></span>
            </div>
          </div>
        </div>
        <div class="to-card">
          <div class="balance">
            ...
            <button
              ...
              :disabled="showReviewBar || isClickAdd"
              @click="addToCart"
              v-if="!alreadyInCart && product.runOut == false"
            >
              Добавить в корзину
            </button>
            <button
            ...
            <button
              ...
              v-if="alreadyInCart && product.runOut == false"
              @click="goRoute($event, 'cart')"
            >
              Перейти в корзину
            </button>
            <p>
              <b>{{ getDeliver(currentDate) }}</b> доставка со склада
            </p>
            <span class="overflow-hidden"
              >Осталось {{ product.quantity }} шт.</span
            >
          </div>
          <div
            v-if="addSuccess"
            class="w-100 text-center mt-4 alert alert-success"
          >
      ...
      <div class="reviews" v-if="product">
        <div class="review-title">
          <div class="main-title">
            <h2><b>Отзывы</b></h2>
            <h5 v-if="product.averageRating > 0">
              Средняя оценка: <b>{{ product.averageRating }}</b>
            </h5>
          </div>
          <button class="write-active-button" @click="showReviewCreate">
            Написать отзыв
          </button>
        </div>
        <h5 class="mt-4" v-if="product.reviews.length == 0">
          Отзывов на этот товар нет
        </h5>
        <div class="box" v-if="product.reviews.length > 0">
          <div class="row ms-lg-1 mt-lg-3 ms-lg-2" ref="reviews">
            <div class="review" v-for="item in product.reviews" :key="item">
              <div class="rev-user">
                ...
                <div class="time">
                  ...
                  <div class="stars">
                    <i
                      ...
                      :key="index"
                      class="me-1 fa fa-star star-rev"
                    ></i>
                  </div>
                </div>
              </div>
          ...
          <div class="buttons mt-4 ms-2" v-if="product.reviews">
            <button
              :disabled="showReviewBar"
              class="scroll-button left"
              @click="scrollLeft('reviews')"
            >
              &#8249;
            </button>
            <button
    ...
    <div
      class="write-review1"
      v-if="showReviewBar"
      style="opacity: 1 !important; z-index: 999"
    >
      ...
      <form class="content" @submit="CreateReview">
        ...
        <button
          class="btn"
          :disabled="!emptyValue || isClickReview"
          :class="{
            opacity: !emptyValue,
          }"
        >
      ...
      <div
        v-if="reviewAlertMessage !== ''"
        class="w-100 mt-4 mb-2 p-2 text-center alert"
        :class="
          this.reviewAlertMessage === 'Ваш отзыв успешно создан'
            ? 'alert-success'
            : 'alert-danger'
